...
C============ setting up the scan ranges ==========================
      IMHL = 125.D0
      IMH5 = RANDOM_RANGE(1.D0,500.D0)
      IMH3 = RANDOM_RANGE(1.D0,500.D0)
      ISH = RANDOM_RANGE(0.05D0,0.35D0)
      IMHH = RANDOM_RANGE(1.D0,500.D0)
      ISA = RANDOM_RANGE(-1.0D0,1.0D0)
      M2 = RANDOM_RANGE(-50.D0,50.D0)
      M1 = RANDOM_RANGE(0.D0,10.D0)
C==================================================================
...
C            THETA = ACOS(VPHI/V)
C            PRINT *, TRIALS," PARAMETERS: "
C     . ,MU2SQ,MU3SQ, MHL,MHH,THETA,LAMBDA1,LAMBDA2,LAMBDA3,
C     . LAMBDA4,LAMBDA5,M1,M2,ALPHA,MH3,MH5
            CALL CALCINDIR
...
C               IF (MHH.GE.MH+0.5D0) THEN
C                  SMLIKE = 1
C               ELSE IF (MHL.LE.MH-0.5D0) THEN
C                  SMLIKE = 2
C               ELSE IF (DABS(KVL).GE.DABS(KVH)) THEN
C                  SMLIKE = 1
C               ELSE
C                  SMLIKE = 2
C               ENDIF
C               IF (SMLIKE.EQ.1) THEN
C                  KV = KVL
C                  KF = KFL
C                  KGAM = KGAML
C                  KZGAM = KZGAML
