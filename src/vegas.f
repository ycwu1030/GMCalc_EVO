C==================================================================
C  EVERYTHING BELOW THIS LINE BELONGS TO VEGAS
C  AND WAS COPIED FROM PROSPINO 2002/02/22.
C==================================================================
...
C     ACC     = 1.D-5
C
C     NO / LONG / SHORT PRINTOUT OF INTEGRALS: IPRINT = 0 / 1 / 10
C     IPRINT = 0
C
C     ...
C
C     CALL INTEG(FUNCT,IDIM,IPOINT,ITER,IPOINT1,ITER1,ACC,RES)
C
C     ...
C
C     STOP
...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE INTEG(FXN,IDIM,IPOINT,ITER,IPOINT1,ITER1,ACC,RES)
...
C***  END OF ADDITION.
      ...
      INDO=1
      DO 1 J=1,NDIM
C===  NEXT LINES MODIFIED ON 2022/07/04 BY HEL TO MODERNIZE DO LOOP TERMINATION
C1     XI(1,J)=ONE
         XI(1,J)=ONE
 1    CONTINUE
...
C===  NEXT LINES MODIFIED ON 2022/07/04 BY HEL TO MODERNIZE DO LOOP TERMINATION
C16    IF(MDS.GE.0)  D(IAJ,J)=D(IAJ,J)+F2
...
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
      IF (DABS(TSI).LT.1.D-35) TSI = DSIGN(1.D-35,TSI)
C--------------------------------------------------------
      WGT=TI2/TSI
      SI=SI+TI*WGT
      SI2=SI2+TI2
      ...
      SCALLS=SCALLS+CALLS
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
      IF (DABS(SWGT).LT.1.D-35) SWGT = DSIGN(1.D-35,SWGT)
      IF (DABS(SI2) .LT.1.D-35) SI2  = DSIGN(1.D-35,SI2)
C--------------------------------------------------------
      AVGI=SI/SWGT
      SD=SWGT*IT/SI2
      CHI2A=0.
...
C-------------------------------------------------------
      ...
      PRINT 202,J
C===  NEXT LINES MODIFIED ON 2022/07/04 BY HEL TO MODERNIZE DO LOOP TERMINATION
C20    PRINT 204,(XI(I,J),DI(I,J),D(I,J),I=1,ND)
      PRINT 204,(XI(I,J),DI(I,J),D(I,J),I=1,ND)
 20   CONTINUE
C===  END OF MODIFICATION
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
21    IF (DABS(AVGI).LT.1.D-35) AVGI = DSIGN(1.D-35,AVGI)
C--------------------------------------------------------
