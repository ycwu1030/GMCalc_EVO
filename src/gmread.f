...
C Identify the deepest of the good extrema:
            ...
            ELSE IF (VV(3).LT.GOODV) THEN
               ...
               GOODV = VV(3)
            ENDIF
            IF ((DABS(VCHI(1)-GOODVCHI).LT.1D-6)
     .           .AND.(DABS(VPHI(1)-GOODVPHI).LT.1D-6)) THEN
               INPUTOK = 1
            ELSE
               INPUTOK = 0
            ENDIF
...
C Read inputs from the user (interactive mode)
     ...
     .        LAMBDA5, M2
      ...
      IF (SH.GT.1.D0.OR.SH.LT.0.D0) THEN
         INPUTOK = 0
      ENDIF

      VCHI = V * SH / DSQRT(8.D0)
      VPHI = DSQRT(V**2 - 8.D0*VCHI**2)
      MU3SQ = MH5**2 - (4.D0*LAMBDA2+LAMBDA5)*VPHI**2/2.D0
     .     - 12.D0*(LAMBDA3 + LAMBDA4)*VCHI**2 - 6.D0*M2*VCHI
      M1 = 4.D0*VCHI/VPHI**2*(MU3SQ + (2.D0*LAMBDA2 - LAMBDA5)*VPHI**2
     .     + 4.D0*(LAMBDA3 + 3.D0*LAMBDA4)*VCHI**2 - 6.D0*M2*VCHI)
      IF (Z2 .EQ. 1) THEN
         M1 = 0.D0
         M2 = 0.D0
         VCHI = 0.D0
      ENDIF
c      M1 = DSQRT(2.D0)*MH5**2/V*SH/CH**2
c     .     - 3.D0/DSQRT(2.D0)*LAMBDA5*V*SH
c     .     - DSQRT(2.D0)*LAMBDA3*V*SH**3/CH**2
c     .     - 6.D0*M2*SH**2/CH**2
c      MU3SQ = 2.D0/3.D0*MH5**2 + M1*VPHI**2/12.D0/VCHI
c     .     - 2.D0*LAMBDA2*VPHI**2 - 28.D0/3.D0*LAMBDA3*VCHI**2
c     .     - 12.D0*LAMBDA4*VCHI**2 - 2.D0*M2*VCHI

      M12SQ = DSQRT(3.D0)/2.D0*VPHI
     .     * (-M1 + 4.D0*(2.D0*LAMBDA2 - LAMBDA5)*VCHI)
      M22SQ = MU3SQ + (2.D0*LAMBDA2 - LAMBDA5)*VPHI**2
     .     + 12.D0*(LAMBDA3+3.D0*LAMBDA4)*VCHI**2 - 12.D0*M2*VCHI
c      M22SQ = M1*VPHI**2/4.D0/VCHI - 6.D0*M2*VCHI
c     .     + 8.D0*(LAMBDA3 + 3.D0*LAMBDA4)*VCHI**2
      LAMBDA1 = 1.D0/8.D0/VPHI**2 * (MHL**2
     .     + M12SQ**2/(M22SQ - MHL**2))
      MU2SQ = -4.D0*LAMBDA1*VPHI**2
     .     - 3.D0*(2.D0*LAMBDA2 - LAMBDA5)*VCHI**2
